...
| **MCC-HIGH-RISK-007** | ≥3 transactions to high-risk MCC merchants (6011 ATM/cash, 6051 quasi-cash, 6211 securities, 4111 transport) | Review period aggregate | — (aggregate metric) | Repeated use of high-risk merchant categories consistent with layering typology |
| **FAN-OUT-008** | ≥25 distinct receiving counterparties in review period | Review period aggregate; 27 distinct counterparties | — (aggregate metric) | Wide dispersion consistent with distribution-stage layering |
| **MULTI-RAIL-009** | Mixed-rail activity: PIX + Card + Wire within review window | PIX (16 txs), Card (10 txs), Wire (1 tx) | — (aggregate metric) | Multi-rail channel diversification consistent with layering typology indicators |
| **CHARGEBACK-010** | Repeat use of merchants with elevated chargeback ratio (≥10%) | Review period aggregate | — (aggregate metric) | Repeated exposure to high-chargeback merchants; counterparty risk indicator |
| **MERCHANT-CONVERGE-011** | Shared receiving merchant with independently flagged subject | Review period; merchants M200460, M200186 shared with C100880 | — (aggregate metric) | Structural convergence point between independently flagged subjects; network linkage indicator |
| **KYC-INCONSISTENCY-012** | PEP=Yes and/or KYC Risk Score ≥80 combined with categorical risk_rating < High or tier = L1 | KYC profile (ongoing); contradictory classification: PEP=Yes, Score=98, Rating=Medium, Tier=L1 | — (KYC-level) | Internal KYC control gap; requires governance review |
